...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>添加分类</h2>
		<div class="manage">
			<form action="${pageContext.request.contextPath}/rest/Category/ShowCategory" method="post">
				<table class="form">
					<tr>
							...
							<select name="dpcParentId">
								...
								<c:forEach var="u" items="${categories}">
									<c:if test="${u.dpcParentId==0}" var="flag">
										<option value="${u.dpcId}">├${u.dpcName}</option>
									</c:if>
									<c:if test="${!flag}">
										<option value="${u.dpcId}">${u.dpcName}</option>
									</c:if>
					...
					<tr>
						...
						<td><input type="text" class="text" name="dpcName"  /></td>
					</tr>
					<tr>
						<td></td>
